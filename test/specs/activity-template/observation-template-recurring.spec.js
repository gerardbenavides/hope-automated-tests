...
/** RECURRING OBSERVATION ACTIVITY */
describe('Practitioner can Add Recurring Observation Activity in the Template', () => {
    ...
    it('Should add Recurring Observation activity', () => {
        PatientActivityPage.addActivity(activity)
    });
    it('Should validate added Recurring Observation activity exists', () => {
        expect(PatientActivityPage.locateActivityByTitle(activity.title)).toBeDisplayed();
    });
    it('Should validate added Recurring Observation activity\'s row details', () => {
        ActivityTemplatePage.validateActivityRowDetails(activity);
    });

});
describe('Practitioner can Edit Recurring Observation Activity in the Template', () => {
    it('Should open the Recurring Observation activity', () => {
        PatientActivityPage.locateActivityByTitle(activity.title).click();
    });
    it('Should edit Recurring Observation activity', () => {
        activity.description = random.paragraph();
        PatientActivityPage.editActivity(activity);
    });
    it('Should validate the edited Recurring Observation activity', () => {
        PatientActivityPage.locateActivityByTitle(activity.editedTitle).click();
        PatientActivityPage.validateActivityDetails(activity);
    });
});
describe('Practitioner can Delete Recurring Observation Activity in the Template', () => {
    it('Should open the edited Recurring Observation activity', () => {
        PatientActivityPage.locateActivityByTitle(activity.editedTitle).click();
    });
    it('Should delete Recurring Observation activity', () => {
        PatientActivityPage.deleteActivity(activity.editedTitle)
    });
    it.skip('Should validate deleted Recurring Observation activity does not exist', () => { // Unable to delete last activity HS30-327
