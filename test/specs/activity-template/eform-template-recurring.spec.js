...
/** RECURRING EFORM ACTIVITY */
describe('Practitioner can Add Recurring eForm Activity in the Template', () => {
    ...
    it('Should add Recurring eForm activity', () => {
        PatientActivityPage.addActivity(activity)
    });
    it('Should validate added Recurring eForm activity exists', () => {
        expect(PatientActivityPage.locateActivityByTitle(activity.title)).toBeDisplayed();
    });
    it('Should validate added Recurring eForm activity\'s row details', () => {
        ActivityTemplatePage.validateActivityRowDetails(activity);
    });

});
describe('Practitioner can Edit Recurring eForm Activity in the Template', () => {
    it('Should open the Recurring eForm activity', () => {
        PatientActivityPage.locateActivityByTitle(activity.title).click();
    });
    it('Should edit Recurring eForm activity', () => {
        activity.description = random.paragraph();
        PatientActivityPage.editActivity(activity);
    });
    it('Should validate the edited Recurring eForm activity', () => {
        PatientActivityPage.locateActivityByTitle(activity.editedTitle).click();
        PatientActivityPage.validateActivityDetails(activity);
    });
});
describe('Practitioner can Delete Recurring eForm Activity in the Template', () => {
    it('Should open the edited Recurring eForm activity', () => {
        PatientActivityPage.locateActivityByTitle(activity.editedTitle).click();
    });
    it('Should delete Recurring eForm activity', () => {
        PatientActivityPage.deleteActivity(activity.editedTitle)
    });
    it.skip('Should validate deleted Recurring eForm activity does not exist', () => { // Unable to delete last activity HS30-327
